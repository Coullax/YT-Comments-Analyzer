...
// POST: Save a YouTube channel URL for the logged-in user
export async function POST(req: NextRequest) {
    try {
        ...
        if (action === 'save') {
            ...
            // Check if channel URL is already saved for this user
        ...
        // Existing functionality: Fetch video IDs
        const channelId = await getChannelIdFromUrl(channelUrl);
        const playlistId = await getUploadPlaylistId(channelId);
        const videoIds = await getVideoIds(playlistId);
...
// PUT: Placeholder for future extensions (unchanged)
