...
export default function YouTubeThumbnails() {
    ...
    // Fetch saved channels on mount and select the first channel if available
    useEffect(() => {
        const fetchChannels = async () => {
            setChannelLoading(true);
            try {
                ...
                if (fetchedChannels.length > 0 && !selectedChannelId) {
                    setSelectedChannelId(fetchedChannels[0].channelId);
                }
            } catch (err: any) {
                toast({
                    title: 'Error',
    ...
    }, [status, toast, selectedChannelId]);
            ...
            <Container maxW="container.xl" py={8}>
                <VStack spacing={8} align="center">
                    ...
                    <Tabs
                        ...
                        w="full"
                        maxW="3xl"
                        index={selectedChannelId ? channels.findIndex((channel) => channel.channelId === selectedChannelId) : channels.length}
                        onChange={(index) => {
                            if (index < channels.length) {
                                setSelectedChannelId(channels[index].channelId);
                            } else {
