...
def perform_detailed_analysis(comments: List[Dict]) -> Dict:
    ...
    try:
        ...
        {{
            ...
            "botComments": [
                {{"text": "comment text", "author": "author name"}},
                {{"text": "comment text", "author": "author name"}}
            ],
            "scamComments": [
                {{"text": "comment text", "author": "author name"}},
        ...
        5. Provide actionable recommendations
        6. Include both top-level comments and replies in the analysis
        7. Identify bot comments based on patterns like repetitive text, unnatural phrasing, or promotional content
        8. Identify scam comments based on suspicious links, financial requests, or misleading claims
        9. Return up to 2 bot and 2 scam comments with their text and author
        Return ONLY the JSON object, no additional text.
        """
        max_retries = 3
        for attempt in range(max_retries):
...
def create_default_section(section_name: str) -> Dict:
    all_defaults = {
        ...
        "recommendations": ["No recommendations available"],
        "positiveInsights": ["No insights available"],
        "futureImprovementsSuggests": ["No suggestions available"],
        "botComments": [],
        "scamComments": []
    }
    if section_name == 'all':
